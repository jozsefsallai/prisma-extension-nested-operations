...
import { buildQueryTargetPath, buildReadTargetPath, buildTargetPath, buildWriteTargetPath, isQueryTarget, isReadTarget, isWriteTarget, targetChainLength, } from "./targets";
...
export function assertOperationChangeIsValid(previousOperation, nextOperation) {
...
export function buildArgsFromCalls(calls, rootParams) {
    ...
    sortedCalls.forEach((call, i) => {
        ...
        if (isWriteTarget(target) && isWriteTarget(origin)) {
            ...
            fieldsByWriteOperation[target.operation];
